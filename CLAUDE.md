...
- `0x20` ROUTING — crosspoint mixer, one packet per output channel (addr
  `0x0Nb7`), carrying that output's full 14-input row. Crosspoint byte =
  `0x80 + percent`; 14 inputs at non-contiguous offsets `[7:13]`+`[23:29]`+
